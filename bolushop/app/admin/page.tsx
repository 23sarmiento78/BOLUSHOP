        ...
        <div className="max-w-7xl mx-auto space-y-12">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex gap-4">
                    <Link href="/admin/products" className="bg-primary text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-primary/20 hover:scale-105 transition-transform flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-500">
                    ...
                    <p className="text-gray-400 font-black uppercase text-[10px] tracking-widest mb-4">Total Productos</p>
                ...
                <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-500">
                    ...
                    <p className="text-gray-400 font-black uppercase text-[10px] tracking-widest mb-4">Órdenes Totales</p>
                ...
                <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-500">
                    ...
                    <p className="text-gray-400 font-black uppercase text-[10px] tracking-widest mb-4">Stock Crítico</p>
                    <div className="flex items-baseline gap-2">
                        <p className="text-4xl font-black text-rose-500">{lowStockProducts.length}</p>
                        <span className="text-[10px] font-black uppercase text-rose-400 bg-rose-50 px-2 py-0.5 rounded-full">Atención</span>
                    </div>
                </div>

                <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-10">
                        <TrendingUp size={80} className="text-primary" />
                    </div>
                    <p className="text-primary font-black uppercase text-[10px] tracking-widest mb-4">Ventas Brutas</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[
                            ...
                            <Link
                                key={action.href}
                                href={action.href}
                                className="group bg-white p-6 rounded-[2rem] border-2 border-transparent hover:border-gray-100 hover:shadow-2xl transition-all duration-300"
                            >
                                ...
                                <p className="font-black text-gray-900 group-hover:text-primary transition-colors">{action.label}</p>
                                    ...
                                    <div key={p.id} className="group flex items-center justify-between gap-4 p-4 rounded-2xl hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-4 truncate">
                                            <div className="relative w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100">
                                                <Image src={p.image || "/bolushop.png"} alt={p.name} fill className="object-cover" />
                                            </div>
                                            <div className="truncate">
                                                <p className="font-bold text-gray-900 truncate">{p.name}</p>
                                                <p className="text-[10px] font-black uppercase tracking-widest text-rose-500">Solo {p.stock} unidades</p>
                                            </div>
                                        </div>
